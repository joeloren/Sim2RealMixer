...
#
# python gym/envs/box2d/noisy_lander.py
#
...
def skew_by(val, rand_state: RandomState = None, max_skew=0.99):
    skew = rand_state.uniform(-1 * max_skew, max_skew) if rand_state else max_skew
    return (1 + skew) * val
...
class NoisyLander(LunarLander):
    ...
    def step(self, action):
        ...
        noisy_main_engine_power = skew_by(val=self.MAIN_ENGINE_POWER, rand_state=self.rand_state,
                                          max_skew=self.max_skew)
        noisy_side_engine_power = skew_by(val=self.SIDE_ENGINE_POWER, rand_state=self.rand_state,
                                          max_skew=self.max_skew)
        ...
        if (self.continuous and action[0] > 0.0) or (not self.continuous and action == 2):
            ...
            p.ApplyLinearImpulse((ox * noisy_main_engine_power * m_power, oy * noisy_main_engine_power * m_power),
                                 impulse_pos,
                                 True)
            self.lander.ApplyLinearImpulse(
                (-ox * noisy_main_engine_power * m_power, -oy * noisy_main_engine_power * m_power),
                impulse_pos, True)
        ...
        if (self.continuous and np.abs(action[1]) > 0.5) or (not self.continuous and action in [1, 3]):
            ...
            p.ApplyLinearImpulse((ox * noisy_side_engine_power * s_power, oy * noisy_side_engine_power * s_power),
                                 impulse_pos,
                                 True)
            self.lander.ApplyLinearImpulse(
                (-ox * noisy_side_engine_power * s_power, -oy * noisy_side_engine_power * s_power),
                impulse_pos, True)
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            self.viewer = rendering.Viewer(self.VIEWPORT_W, VIEWPORT_H)
